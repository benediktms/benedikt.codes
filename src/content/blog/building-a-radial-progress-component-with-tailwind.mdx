...
I've been toying around with building a task and time mangament app recently.
I'm using it as a way to get into learning a bit more about
[Svelte(Kit)](https://svelte.dev/) and [Tauri](https://tauri.app/). I've also
increasingly enjoyed using [Tailwind]() and
[shadcn/ui](https://ui.shadcn.com/) for styling so I was quite happy to see
that the project is being ported to other UI frameworks, including svelte's
[port](https://www.shadcn-svelte.com/). However, something that is missing
from both the svelte port and the original project are radial progress
components. One great example I found was [daisyUI's
implementation](https://daisyui.com/components/radial-progress/). I didn't
however want to add a new dependency just for a single component. So I decided
to try my hand at building my own.
